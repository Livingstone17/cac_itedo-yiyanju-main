      ...
      return (
        <div key={item.label} className="relative" ref={ref}>
          ...
          >
            ...
            <ChevronDown
              className={`h-4 w-4 transition-transform duration-200 ${openDropdown === dropdownType ? "rotate-180" : ""
                }`}
            />
          </button>
          ...
          {item.label === "Live Stream" && openDropdown === "live" && (
            <div className="absolute top-full mt-2 w-40 rounded-lg border border-border bg-popover text-popover-foreground shadow-lg">
              <a
                href="/listen/video"
                className="flex items-center px-4 py-2 text-sm text-muted-foreground hover:bg-muted hover:text-foreground first:rounded-t-lg"
              ...
              <a
                href="/listen/audio"
                className="flex items-center px-4 py-2 text-sm text-muted-foreground hover:bg-muted hover:text-foreground last:rounded-b-lg"
                onClick={() => setOpenDropdown(null)}
              >
                <Mic className="mr-2 h-4 w-4" />
                Listen Live
          ...
          {item.label === "Resources" && openDropdown === "resources" && (
            <div className="absolute top-full mt-2 w-52 rounded-lg border border-border bg-popover text-popover-foreground shadow-lg">
                ...
                if (resourceItem.external) {
                  return (
                    <a
                      ...
                      className="flex items-center px-4 py-2.5 text-sm text-muted-foreground first:rounded-t-lg last:rounded-b-lg hover:bg-muted hover:text-foreground"
                      onClick={() => setOpenDropdown(null)}
                    >
        ...
        {isMenuOpen && (
          <div className="border-t border-border py-4 md:hidden">
            <div className="flex flex-col space-y-4">
              {navigationItems.map(renderMobileItem)}
            </div>
          </div>
        )}
      </div>
